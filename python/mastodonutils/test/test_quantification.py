...
from mastodonutils.FTA import Event,FaultTree,Quantification
...
class TestQuantification(unittest.TestCase):
    ...
    def testInputs(self):
        ...
        with self.assertRaises(IOError) as cm:
            Quantification(logic='not_a_valid_filename.txt',bas_events='not_a_valid_filename.txt',hazard='not_a_valid_filename.txt')
        ...
        inputs = Quantification('SSI_logic.txt', 'bas_events_ln.txt', analysis='Fragility',hazard='hazard.txt', IM=[0.1, 4], nbins=15)
        self.assertEqual(inputs.logic, [['TE', 0, ['IE3', 'IE4']],['IE4', 0, ['C4']],['IE3', 0, ['C3', 'IE2']],
                        ['IE2', 1, ['C2', 'IE1']],['IE1', 0, ['C1']]])
        self.assertEqual(inputs.basic_events, [['C1', 'LNORM', [1.88, 0.5]], ['C2', 'LNORM', [3.78, 0.79]],
                        ['C3', 'LNORM', [2.33, 0.76]], ['C4', 'LNORM', [3.66, 0.45]]])
        ...
        self.assertEqual(inputs.IM,[0.1,4])
        ...
        self.assertEqual("The supplied value of nbins must be a positive integer.", str(cm.exception))

        # Inputs for Risk
        inputs = Quantification('events.txt', 'bas_events.txt', analysis='Risk')
        self.assertEqual(inputs.logic, [['TOP', 1, ['GATE1', 'GATE2']], ['GATE1', 0, ['FT-N/m-1', 'FT-N/m-2', 'FT-N/m-3']], ['GATE2', 0, ['B1', 'B3', 'B4']], [
                         'GATE3', 0, ['B2', 'B4']], ['FT-N/m-1', 1, ['GATE3', 'B3', 'B5']], ['FT-N/m-2', 1, ['GATE3', 'B1']], ['FT-N/m-3', 1, ['B3', 'B5', 'B1']]])
        self.assertEqual(inputs.basic_events, [['B1', 'PE', [0.01]], ['B2', 'PE', [0.02]], [
                         'B3', 'PE', [0.03]], ['B4', 'PE', [0.04]], ['B5', 'PE', [0.05]]])
        ...
        inputs = Quantification('events.txt', 'bas_events.txt', analysis='Risk',uncertainty=True,nsamp=1000,seed=436546754)
        self.assertEqual(inputs.uncertainty, True)
        self.assertEqual(inputs.nsamp, 1000)
        self.assertEqual(inputs.seed, 436546754)

        with self.assertRaises(TypeError) as cm:
            Quantification('events.txt', 'bas_events.txt', analysis='Risk',uncertainty=True,nsamp=[1],seed=42.1)
        self.assertEqual("The supplied value of nsamp must be an integer.", str(cm.exception))

        with self.assertRaises(TypeError) as cm:
            Quantification('events.txt', 'bas_events.txt', analysis='Risk',uncertainty=True,nsamp=10,seed=42.0)
        self.assertEqual("The supplied value of seed must be an integer.", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            Quantification('events.txt', 'bas_events.txt', analysis='Risk',uncertainty=True,nsamp=-10,seed=42.0)
        self.assertEqual("The supplied value of nsamp must be a positive integer.", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            Quantification('events.txt', 'bas_events.txt', analysis='Risk',uncertainty=True,nsamp=10,seed=-42)
        self.assertEqual("The supplied value of seed must be a positive integer.", str(cm.exception))

    def testTOPrisk(self):
        FTA = Quantification('events.txt', 'bas_events_norm.txt', analysis='Risk')
        self.assertRisk(FTA)
        self.assertIMratio(FTA.beIMratio)
        self.assertIMdiff(FTA.beIMdiff)

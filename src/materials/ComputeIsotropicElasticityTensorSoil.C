...
InputParameters validParams<ComputeIsotropicElasticityTensorSoil>() {
  ...
  params.addRequiredParam<std::vector<Real>>(
      "poissons_ratio", "Vector of Poisson's ratio values that map one-to-one "
                        "with the number 'layer_ids' parameter.");
  params.addRequiredParam<std::vector<Real>>(
      "shear_modulus", "Vector of  shear modulus values that map one-to-one "
                       "with the number 'layer_ids' parameter.");
  params.addRequiredParam<std::vector<Real>>(
...
void ComputeIsotropicElasticityTensorSoil::computeQpElasticityTensor() {

  _P_wave_modulus = _layer_shear_modulus[_qp] * 2.0 *
                    (1.0 - _layer_poissons_ratio[_qp]) /
  ...
  if (_wave_speed_calculation) {
    ...
    (*_shear_wave_speed)[_qp] =
        std::sqrt(_layer_shear_modulus[_qp] / _density[_qp]);
  ...
  std::vector<Real> iso_const(2);
  iso_const[0] =
      _P_wave_modulus - 2.0 * _layer_shear_modulus[_qp]; // lambda = M - 2G

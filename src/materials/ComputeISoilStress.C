...
template <> InputParameters validParams<ComputeISoilStress>() {
  ...
  params.addRequiredParam<std::vector<std::vector<Real>>>(
      "yield_stress", "Yield stress for the individual stress-strain curves "
                      "for each soil layer.");
  params.addParam<Real>(
      "b_exp", "The exponent for pressure dependent stiffness calculation.");
  params.addParam<Real>("a0", "The first coefficient for pressure dependent "
                              "yield strength calculation.");
  params.addParam<Real>("a1", "The second coefficient for pressure dependent "
                              "yield strength calculation.");
  params.addParam<Real>("a2", "The third coefficient for pressure dependent "
                              "yield strength calculation.");
  ...
  params.addRequiredParam<std::vector<std::vector<Real>>>(
      "youngs_modulus", "The youngs modulus for the individual stress-strain "
                        "curves for each soil layer.");
  params.addRequiredParam<std::vector<Real>>(
      "poissons_ratio", "The Poissons's ratio for each soil layer.");
  params.addRequiredParam<std::vector<std::string>>(
      "base_models", "Base name for each elastic perfectly plastic model. All "
  ...
  params.addParam<bool>(
      ...
      "Set to False to turn off P and S wave speed calcualtion.");
  return params;
}
...
ComputeISoilStress::ComputeISoilStress(const InputParameters &parameters)
    : ComputeFiniteStrainElasticStress(parameters),
      ...
      _b_exp(getParam<Real>("b_exp")), _a0(getParam<Real>("a0")),
      _a1(getParam<Real>("a1")), _a2(getParam<Real>("a2")),
      ...
      _strength_pressure_correction(1.0), _stiffness_pressure_correction(1.0),
      ...
      _shear_wave_speed(_wave_speed_calculation
                            ? &declareProperty<Real>("shear_wave_speed")
                            : NULL),
      _P_wave_speed(_wave_speed_calculation
                        ? &declareProperty<Real>("P_wave_speed")
                        : NULL),
      _density(_wave_speed_calculation ? &getMaterialProperty<Real>("density")
                                       : NULL),
      _tangent_modulus(0.0), _pos(0) {
  if ((_youngs.size() != _layer_ids.size()) ||
      (_yield_stress.size() != _layer_ids.size()) ||
  ...
  for (unsigned int i = 0; i < _base_models.size(); i++) {
    _stress_model[i] =
        &declareProperty<RankTwoTensor>(_base_models[i] + "_stress_model");
    _stress_model_old[i] = &getMaterialPropertyOld<RankTwoTensor>(
        _base_models[i] + "_stress_model");
  }
...
void ComputeISoilStress::initQpStatefulProperties() {
  ...
  _current_id =
      static_cast<unsigned int>(std::round(_soil_layer_variable[_qp]));
  // Get the position of the current id in the layer_ids array
  _pos = find(_layer_ids.begin(), _layer_ids.end(), _current_id) -
         _layer_ids.begin();
  ...
  if (_wave_speed_calculation) {
    ...
    (*_shear_wave_speed)[_qp] =
        std::sqrt(initial_youngs / (2.0 * (1.0 + _poissons_ratio[_pos])) /
                  (*_density)[_qp]);

    // P wave speed is sqrt(P wave modulus/density)
    (*_P_wave_speed)[_qp] =
        std::sqrt(initial_youngs * (1.0 - _poissons_ratio[_pos]) /
                  (1.0 + _poissons_ratio[_pos]) /
  ...
  for (unsigned int i = 0; i < _base_models.size(); i++) {
    ...
    if (residual_vertical != 0.0) {
      ...
      (*_stress_model[i])[_qp](2, 2) =
          residual_vertical * _youngs[_pos][i] / sum_youngs;
      (*_stress_model[i])[_qp](0, 0) =
          residual_xx * _youngs[_pos][i] / sum_youngs;
      (*_stress_model[i])[_qp](1, 1) =
          residual_yy * _youngs[_pos][i] / sum_youngs;
      ...
      if (dev_stress_model >
          _yield_stress[_pos][i] * _strength_pressure_correction)
        dev_model *= (_yield_stress[_pos][i] * _strength_pressure_correction) /
                     dev_stress_model;

      (*_stress_model[i])[_qp] =
          dev_model * _youngs[_pos][i]; // stress_model contains only the
                                        // deviatoric part of the stress
    }
    residual_vertical =
        residual_vertical - (*_stress_model[i])[_qp](2, 2) - mean_pressure;
...
void ComputeISoilStress::computeQpStress() {
  ...
  _stress_new.zero();
  computeStress();
  _stress[_qp] = _rotation_increment[_qp] * _stress_new *
  ...
  _Jacobian_mult[_qp] = _elasticity_tensor[_qp] *
                        _tangent_modulus; // This is NOT the exact jacobian
}

void ComputeISoilStress::computeStress() {
  if (_t_step == 0)
    return;
  ...
  for (unsigned int i = 0; i < _base_models.size(); i++) {
    ...
    _individual_stress_increment =
        _elasticity_tensor[_qp] * (_strain_increment[_qp]);

    // calculate pressure for each element
    mean_pressure += _individual_stress_increment.trace() / 3.0 *
    ...
    _deviatoric_trial_stress =
        _individual_stress_increment.deviatoric() *
            _stiffness_pressure_correction +
    ...
    if (yield_condition > 0.0)
      _deviatoric_trial_stress *= _yield_stress[_pos][i] *
                                  _strength_pressure_correction /
                                  effective_trial_stress;
    else

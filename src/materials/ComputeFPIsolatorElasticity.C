...
validParams<ComputeFPIsolatorElasticity>()
{
  ...
  params.addClassDescription("Compute the forces and the stiffness matrix for a single concave Friction Pendulum isolator element.");
  // Switches
  params.addRequiredParam<bool>("pressure_dependent", "Switch for modeling friction dependence on the instantaneous pressure.");
  params.addRequiredParam<bool>("temperature_dependent", "Switch for modeling friction dependence on the instantaneous temperature.");
  params.addRequiredParam<bool>("velocity_dependent", "Switch for modeling friction dependence on the instantaneous sliding velocity.");
  // Material properties
  ...
  params.addRequiredParam<Real>("unit", "Unit conversion for pressure to be used in the pressure factor computation.");
  ...
  params.addParam<Real>("k_x", 10e13, "Stiffness of the bearing in translation along x axis. Defaults to 10e13 (SI units)");
  params.addParam<Real>("k_xx", 10e13, "Stiffness of the bearing in rotation about x axis. Defaults to 10e13 (SI units) ");
  params.addParam<Real>("k_yy", 10e13, "Stiffness of the bearing in rotation about y axis. Defaults to 10e13 (SI units) ");
  params.addParam<Real>("k_zz", 10e13, "Stiffness of the bearing in rotation about z axis. Defaults to 10e13 (SI units) ");
  params.addParam<Real>("tol", 1e-8, "Convergence tolerance to satisfy equilibrium of element. Defaults to 1e-8");
  params.addParam<Real>("maxiter", 100, "Maximum number of iterations for equilibrium. Defaults to 100.");
...
ComputeFPIsolatorElasticity::computeShear()
{
  ...
  if (_temperature_dependent)
  {
    ...
    if (_t_step >= 1 && resultantU > 0.0)
    {
      ...
      for (int jTemp=1; jTemp<=_t_step; jTemp++)
      {
        ...
        temperature_change=temperature_change+sqrt(_diffusivity/_pi)*_vec_heatflux[_t_step-jTemp]*dt_analysis/(_conductivity*sqrt(tau));   // Solving the integral for temperature increment
      }

      _temperature_center = 20.0+temperature_change;             // Temperature at the center of the sliding surface

      _ktF = 0.789*(pow(0.7, (_temperature_center/50.0))+0.4);   // Update the temperature factor
    }
  }
  ...
  do
  {
    ...
    else
    {
      ...
      // Set shear forces
      _Fb[_qp](1,0) = qYield*_qTrial(0)/qTrialNorm - N*_local_def[_qp](5,0) + k2y*_basic_def[_qp](1,0);
      _Fb[_qp](2,0) = qYield*_qTrial(1)/qTrialNorm + N*_local_def[_qp](4,0) + k2z*_basic_def[_qp](2,0);
  ...
  } while (sqrt(pow(_Fb[_qp](1,0)-_qbOld(0),2)+pow(_Fb[_qp](2,0)-_qbOld(1),2) >= _tol) && (iter <= _maxiter));

  // Issue warning if iteration did not converge
  if (iter >= _maxiter)
  {
    mooseError("Error: ComputeFPIsolatorElasticity::computeshearforce() - did not find the shear force after "
    ,iter, " iterations and norm: ",sqrt(pow(_Fb[_qp](1,0)-_qbOld(0),2)+pow(_Fb[_qp](2,0)-_qbOld(1),2)), ".\n");
  }
}

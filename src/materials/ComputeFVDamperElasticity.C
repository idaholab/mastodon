...
ComputeFVDamperElasticity::validParams()
{
  ...
  // Input parameters
  params.addRequiredRangeCheckedParam<Real>("cd", "cd > 0.0", "Damping co-efficient.");
...
{
  _vel[_qp] = _vel_old[_qp] + (_gamma / _beta) * (((_basic_def[_qp] - _basic_def_old[_qp]) / _dt) -
                                                  (_vel_old[_qp]));
  ...
  if (_vel[_qp] != 0)
  {
    Real k_damp = _cd * MathUtils::sign(_vel[_qp]) * _alpha * pow(fabs(_vel[_qp]), _alpha - 1.0) *
                  (_gamma / _beta) / _dt;
    Real k_spring = _k;
...
void
ComputeFVDamperElasticity::dormandPrince(
    Real t_vel, Real t_vel_new, Real t_fd, Real h, Real & yt, Real & eps, Real & error)
{
  Real k1 = fdot(t_vel, t_fd) * h;
  Real k2 = fdot((t_vel_new - t_vel) * (1.0 / 5.0) + t_vel, t_fd + (1.0 / 5.0) * k1) * h;
  Real k3 = fdot((t_vel_new - t_vel) * (3.0 / 10.0) + t_vel,
                 t_fd + (3.0 / 40.0) * k1 + (9.0 / 40.0) * k2) *
            h;
  Real k4 = fdot((t_vel_new - t_vel) * (4.0 / 5.0) + t_vel,
                 t_fd + (44.0 / 45.0) * k1 + (-56.0 / 15.0) * k2 + (32.0 / 9.0) * k3) *
            h;
  Real k5 = fdot((t_vel_new - t_vel) * (8.0 / 9.0) + t_vel,
                 t_fd + (19372.0 / 6561.0) * k1 + (-25360.0 / 2187.0) * k2 +

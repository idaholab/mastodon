...
template <> InputParameters validParams<UniformLayerAuxKernel>() {
  ...
  params.addRequiredParam<std::vector<Real>>(
      "interfaces", "A list of layer interface locations to apply across the "
                    "domain in the specified direction.");
  params.addParam<std::vector<unsigned int>>(
      "layer_ids", "A list of layer identifiers to assign to each interface. "
  ...
  params.addParam<RealVectorValue>("direction", RealVectorValue(1, 0, 0),
                                   "The direction to apply layering.");
...
UniformLayerAuxKernel::UniformLayerAuxKernel(const InputParameters &parameters)
  ...
  if (_direction.norm() == 0)
    mooseError(
        "The supplied direction vector is not valid, it has a zero norm.");
  ...
  if (_layer_ids.size() != _interfaces.size())
    mooseError(
        "The number of 'interfaces' must match the number of 'layer_ids'.");
}

Real UniformLayerAuxKernel::computeValue() {

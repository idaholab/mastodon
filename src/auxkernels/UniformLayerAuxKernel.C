...
InputParameters validParams<UniformLayerAuxKernel>()
{
  ...
  params.addRequiredParam<std::vector<Real>>("interfaces", "A list of layer interface locations to apply across the domain in the specified direction.");
  params.addParam<std::vector<unsigned int>>("layer_ids", "A list of layer identifiers to assign to each interface. If not provided integer values starting from 0 are utilized, if provided the length of this vector must be identical to the 'intefaces' vector.");
  params.addParam<RealVectorValue>("direction", RealVectorValue(1, 0, 0), "The direction to apply layering.");
  params.addClassDescription("Computes an AuxVariable for representing a layered structure in an arbitrary direction.");
  return params;
}
...
{
  // Compute projected distance (dot product of direction vector and the current point)
  if (_nodal)
  ...
  // Locate the layer
  std::vector<Real>::const_iterator iter = std::upper_bound(_interfaces.begin(), _interfaces.end(), _distance);
  if (iter == _interfaces.end())
    mooseError("Failed to locate an interface within the domain.");

  // Return the "layer_id" at the same location as the interface that was located.

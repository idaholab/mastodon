...
validParams<Fragility>()
{
  ...
  params.addRequiredParam<std::string>("hazard_multiapp", "Name of the multiapp corresponding to ground motion sampling.");
  params.addRequiredParam<std::string>("probabilistic_multiapp", "Name of the multiapp corresponding to the probabilistic simulations.");
  params.addRequiredParam<unsigned int>("num_gms", "Number of ground motions used in each intensity bin.");
  params.addRequiredParam<std::string>("demand_variable", "Demand variable for the SSC that is also column name in the output csv file. Acceleration variable only."); // TODO: generalize class for all demand variables by processing VectorPostprocessor data and getting rid of response spectra calculation in this class.
  params.addRequiredParam<Real>("frequency", "Frequency at which, the spectral demand of the SSC is calculated.");
  params.addParam<Real>("damping_ratio", 0.05, "Damping ratio at which the spectral demand of the SSC is calculated. Default is 0.05");
  params.addRequiredParam<Real>("dt", "dt for response spectra calculation.");
  params.addRequiredParam<Real>("median_capacity", "Median capacity of the SSC in terms of local demand at the SSC location.");
  params.addRequiredParam<Real>("beta_capacity", "Uncertainty in the capacity of the SSC.");
  params.addRequiredParam<unsigned int>("num_bins", "Number of bins in the hazard curve where the risk calculation is performed.");
  params.addRequiredParam<unsigned int>("num_samples", "Number of probabilistic simulations for each bin.");
  params.addRequiredParam<std::vector<Real>>("im_values", "IM values used in the bins."); //TODO: remove this later by transferring IM values from hazard curve UO
  params.addRequiredParam<std::vector<Real>>("median_fragility_limits", "Limits for median fragility of the component.");
  params.addRequiredParam<std::vector<Real>>("beta_fragility_limits", "Limits for the lognormal standard deviation of the component fragility.");
  params.addClassDescription("Calculate the seismic fragility of an SSC by postprocessing the results of a probabilistic or stochastic simulation.");
  return params;
}
    ...
    _beta_fragility(declareVector("fragility_beta"))
{
  ...
  mooseError("In Fragility block '", name(), "'. The Fragility block requires that libMesh be compiled with an external Boost library, this may be done "
  ...
  if (_median_cap <= 0 || _beta_ssc_cap <= 0)
    mooseError("Error in block '" + name() + "'. Median and lognormal standard deviation of the capacity of the SSC must be positive.");
  ...
  if (_median_fragility_limits.size() != 2 || _beta_fragility_limits.size() != 2)
    mooseError("Error in block '" + name() + "'. Please provide two limits (minimum and maximum) for median and lognormal standard deviation of the component.");
  if (_median_fragility_limits[0] >= _median_fragility_limits[1])
    mooseError("Error in block '" + name() + "'. Minimum median fragility should be less than the maximum median fragility.");
  if (_beta_fragility_limits[0] >= _beta_fragility_limits[1])
    mooseError("Error in block '" + name() + "'. Minimum should be less than the maximum for fragility lognormal standard deviation.");
  if (_median_fragility_limits[0] <= 0 || _beta_fragility_limits[0] <=0)
    mooseError("Error in block '" + name() + "'. Limits of median and beta of the component fragility must be positive.");
  // Check if _im_values is of the same size as the number of bins
  if (_im_values.size() != _num_bins)
    mooseError("Error in block '" + name() + "'. Number of IM values should be the same as the number of bins.");
}
...
Fragility::execute()
{
  ...
  for (std::size_t bin=0; bin<_num_bins; bin++)
  {
    ...
    std::vector<Real> stoc_demands = Fragility::calcDemandsFromFile(bin); // calculating demands from csv files if it is a restart PRA
    ...
    boost::math::lognormal_distribution<Real> demand_distribution(log(_median_demand[bin]), _beta_demand[bin]);
    boost::math::lognormal_distribution<Real> capacity_distribution(log(_median_cap), _beta_ssc_cap);
    _conditional_pf[bin] = MastodonUtils::greaterProbability<boost::math::lognormal>(demand_distribution, capacity_distribution);
  ...
  std::vector<Real> fitted_vals = MastodonUtils::maximizeLogLikelihood(_im, _conditional_pf, _median_fragility_limits, _beta_fragility_limits, 500);
  ...
  for (unsigned int i = 0; i < _num_gms; ++i)
  {
    for (unsigned int j = 0; j < _num_samples; ++j)
    {
      demand_sample_filename = _master_file + "_out_" + _hazard_multiapp +
                             ...
                             MastodonUtils::zeropad(j, _num_samples) +
                             ".csv";

      _console<<"In block '" + name() + "'. Reading file: "<<demand_sample_filename<<std::endl;
      ...
      demand_sample = MastodonUtils::regularize(demand_sample, demand_time, _dtsim)[1]; //regularize the demand sample
      demand_sample_spectrum = MastodonUtils::responseSpectrum(0.01, 100, 401, demand_sample, _ssc_xi, _dtsim);

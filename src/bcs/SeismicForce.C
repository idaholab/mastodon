...
template <> InputParameters validParams<SeismicForce>() {
  ...
  params.addRequiredParam<unsigned int>(
      "vel_component", "The direction in which the input velocity is applied.");
  params.addParam<Real>("factor", 1.0,
                        "Scaling factor to be applied to the force.");
  params.addParam<FunctionName>(
      "velocity_function",
      "The function that describes the input ground velocity.");
  params.addCoupledVar("velocity",
                       "The variable that describes the input velocity.");
...
SeismicForce::SeismicForce(const InputParameters &parameters)
    : IntegratedBC(parameters), _component(getParam<unsigned int>("component")),
      ...
      _factor(getParam<Real>("factor")), _alpha(getParam<Real>("alpha")),
  ...
  if (_component >= _mesh.dimension())
    mooseError("Invalid value for 'component' (", _component, ") given in \"",
               name(), "\" block, it must be a value from 0 to ",
               _mesh.dimension() - 1, ".");

  if (_vel_component >= _mesh.dimension())
    mooseError("Invalid value for 'vel_component' (", _vel_component,
               ") given in \"", name(),
               "\" block, it must be a value from 0 to ", _mesh.dimension() - 1,
               ".");
...
Real SeismicForce::computeQpResidual() {
  ...
  if (isParamValid("velocity_function")) {
    ...
    vel[_vel_component] =
        velocity_function->value(_t + _alpha * _dt, _q_point[_qp]);
  } else {

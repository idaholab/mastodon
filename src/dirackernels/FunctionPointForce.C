...
template <> InputParameters validParams<FunctionPointForce>() {
  ...
  params.addParam<Real>(
      "alpha", 0.0,
      "The Hilber Hughes Taylor (HHT) time integration parameter.");
  ...
  params.addRequiredParam<FunctionName>(
      "function",
      "The function defining the force as a function of space and/or time");
  return params;
}
...
FunctionPointForce::FunctionPointForce(const InputParameters &parameters)
  ...
  if (isParamValid("x_position") && !isParamValid("number"))
    mooseError("The 'number' parameter is required in the \"", name(),
...
void FunctionPointForce::addPoints() {
...
Real FunctionPointForce::computeQpResidual() {

...
template <> InputParameters validParams<SeismicSource>() {
  ...
  params.addRequiredParam<Real>("strike",
                                "The strike angle of the fault in degrees.");
  params.addRequiredParam<Real>("dip",
                                "The dip angle of the fault in degrees.");
  params.addRequiredParam<Real>("rake",
                                "The rake angle of the fault in degrees.");
  params.addRequiredParam<Real>("area",
                                "The area over which slip is distributed.");
  params.addRequiredParam<FunctionName>(
      "slip", "The function describing slip as a function of time.");
  params.addRequiredParam<Real>(
      "shear_modulus",
      "The shear modulus of the soil around the seismic source.");
  params.addParam<Real>(
      "alpha", 0, "The Hilber Hughes Taylor (HHT) time integration parameter.");
  params.addRequiredParam<unsigned int>(
      "component", "The direction in which the force is applied.");
  params.addParam<std::vector<Real>>("point",
                                     "The x,y,z coordinates of the source.");
  params.addParam<FunctionName>(
      "x_position", "The function containing the x coordinates of the points. "
                    "The first column gives source number (starting with 1) "
                    "and second column gives x coordinate.");
  params.addParam<FunctionName>(
      "y_position", "The function containing the y coordinates of the points. "
                    "The first column gives source number (starting with 1) "
                    "and second column gives y coordinate.");
  params.addParam<FunctionName>(
      "z_position", "The function containing the z coordinates of the points. "
                    "The first column gives source number (starting with 1) "
...
SeismicSource::SeismicSource(const InputParameters &parameters)
    : DiracKernel(parameters), _component(getParam<unsigned int>("component")),
      _alpha(getParam<Real>("alpha")),
      _shear_modulus(getParam<Real>("shear_modulus")),
      _area(getParam<Real>("area")), _slip_function(&getFunction("slip")),
      _moment_magnitude(0.0), _force(0.0) {
  ...
  if (_component >= _mesh.dimension())
    mooseError("Error in " + name() +
               ". component cannot exceed mesh dimensions.");
  ...
  _moment[0][2] = -(cos(dip) * cos(rake) * cos(strike) +
...
Real SeismicSource::computeQpResidual() {
  /**
  ...
  _moment_magnitude =
      _shear_modulus * _area * _slip_function->value(_t + _alpha * _dt, _qp);

...
template <> InputParameters validParams<SeismicForceAction>() {
  ...
  params.addClassDescription(
      "This action sets up Seismic forces boundary conditions.");

  params.addRequiredParam<std::vector<VariableName>>(
      "displacements", "The displacement variables for the problem.");
  ...
  params.addParam<std::vector<FunctionName>>(
      "velocity_functions", "The vector of function names that describes the "
                            "input ground motions. Must be same size as "
                            "input_component.");
  params.addParam<Real>(
      "alpha", 0.0,
      "The alpha parameter required for HHT time integration scheme.");
  return params;
}
...
SeismicForceAction::SeismicForceAction(const InputParameters &params)
    : Action(params),
      ...
      _velocity_ptrs(isParamValid("velocities")
                         ? &getParam<std::vector<VariableName>>("velocities")
                         : NULL),
      _velocity_function_ptrs(
          isParamValid("velocity_functions")
  ...
  if (_velocity_function_ptrs &&
      (_input_components.size() != _velocity_function_ptrs->size()))
    mooseError("The number of components listed in 'input_components' must "
...
void SeismicForceAction::act() {
  ...
  for (unsigned int j = 0; j < _input_components.size(); ++j) {
    ...
    if (_velocity_function_ptrs)
      params.set<FunctionName>("velocity_function") =
          (*_velocity_function_ptrs)[j];
    else
      params.set<std::vector<VariableName>>("velocity") = {
          (*_velocity_ptrs)[j]};

    for (unsigned int i = 0; i < _displacements.size(); ++i) {
      std::stringstream obj_name;
      obj_name << prefix << "_" << name() << "_" << i << "_"
               << _input_components[j];

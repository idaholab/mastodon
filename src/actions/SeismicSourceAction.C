...
InputParameters validParams<SeismicSourceAction>()
{
  ...
  params.addRequiredParam<std::vector<NonlinearVariableName> >("displacements", "The vector of displacement variables.");
  ...
  params.addParam<std::vector<FunctionName> >("position_function", "The vector of function names that describes the x, y and z coordinates of the source.");
  params.addParam<unsigned int>("number", "The number of point sources. This number should be same as number of entries in the function files describing the position of the point source.");
  params.addParam<std::vector<Real> >("point", "The x, y and z coordinate of a single point source.");
...
SeismicSourceAction::act()
{
  ...
  if (isParamValid("position_function") && isParamValid("number"))
  {
    ...
    std::vector<FunctionName> position_function = getParam<std::vector<FunctionName> >("position_function");

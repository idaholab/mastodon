...
template <> InputParameters validParams<SeismicSourceAction>() {
  ...
  params.addClassDescription(
      "Set up Seismic source dirac kernels in multiple directions.");
  ...
  params.addParam<std::vector<FunctionName>>(
      "position_function", "The vector of function names that describes the x, "
                           "y and z coordinates of the source.");
  params.addParam<unsigned int>(
      "number", "The number of point sources. This number should be same as "
                "number of entries in the function files describing the "
                "position of the point source.");
  params.addParam<std::vector<Real>>(
      "point", "The x, y and z coordinate of a single point source.");
  params.addParam<Real>("alpha", 0.0,
                        "The Hilber Hughes Taylor time integration parameter.");
  return params;
}

SeismicSourceAction::SeismicSourceAction(const InputParameters &params)
    : Action(params) {}

void SeismicSourceAction::act() {
  ...
  if (isParamValid("displacements"))
    ...
    mooseError("Error in " + name() +
               ". A string of displacements is required as input.");
  ...
  if (isParamValid("point"))
    params.set<std::vector<Real>>("point") =
        getParam<std::vector<Real>>("point");

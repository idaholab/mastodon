...
template <> InputParameters validParams<SeismicDisplacementAction>() {
  ...
  params.addClassDescription(
      "This action sets up seismic displacement boundary conditions.");
  ...
  params.addRequiredParam<std::vector<VariableName>>(
      "accelerations", "The vector of acceleration variables that are coupled "
                       "to displacements. The size of this vector should be "
                       "same as displacements.");
  params.addRequiredParam<std::vector<VariableName>>(
      "velocities", "The vector of velocity variables names. The input "
                    "velocities can be obtained from this variable. The size "
                    "of this vector must be same size as input_component.");
  params.addRequiredParam<std::vector<unsigned int>>(
      "input_components",
      "The directions in which the input ground motions are applied.");
  params.addRequiredParam<std::vector<FunctionName>>(
      "input_functions", "The vector of function names that describes the "
                         "input ground motions. Must be same size as "
                         "input_component.");
  params.addParam<Real>("beta", 0.0,
                        "The beta parameter for newmark time integration.");

  return params;
}

SeismicDisplacementAction::SeismicDisplacementAction(
    ...
    : Action(params),
      ...
      _input_components(
          getParam<std::vector<unsigned int>>("input_components")),
      _input_functions(getParam<std::vector<FunctionName>>("input_functions")) {
  if ((_displacements.size() != _input_components.size()) ||
      (_displacements.size() != _accelerations.size()) ||
      (_displacements.size() != _velocities.size()) ||
      _displacements.size() != _input_functions.size())
    mooseError("In the \"", name(), "\" block the 'displacements', "
                                    "'velocities', 'accelerations', "
                                    "'input_functions', and 'input_components' "
                                    "should be of the same size.");
}

void SeismicDisplacementAction::act() {
  ...
  for (unsigned int j = 0; j < _input_components.size(); ++j) {
    ...
    _problem->addBoundaryCondition("PresetDisplacement", obj_name.str(),
                                   params);
  }
}

...
void assertTree(std::map<std::string, FTAUtils::FaultTree::_node *> tree_node)
{
    // TOP
...
void assertMocus(std::set<std::set<std::string>> tree_mocus){
    ...
    // print mocus    
    /*
    for (it_tree_mocus = tree_mocus.begin();
         it_tree_mocus != tree_mocus.end();
         ++it_tree_mocus) {

        tree_mocus_children = *it_tree_mocus;

        for (it_tree_mocus_children = tree_mocus_children.begin();
             it_tree_mocus_children != tree_mocus_children.end(); 
             ++it_tree_mocus_children) {

                 std::cout << " " << *it_tree_mocus_children;
    ...
    */
    ...
    std::set<std::set<std::string>> matrix_gold = {{"B2", "B1"},
                                                   {"B4", "B1"},
                                                   {"B1", "B5", "B3"},
                                                   {"B5", "B2", "B3"},
                                                   {"B4", "B5", "B3"}};
...
std::map<std::string, FTAUtils::FaultTree::_node *> eventList(){
...
// For input
std::vector<FileName> file_lists_fault_tree_1,
                      file_lists_fault_tree_2,
                      file_lists_fault_tree_3;
...
TEST(FTAUtils, FaultTree)
{
    ...
    try
    {
        ...
        FTAUtils::FaultTree(file_lists_fault_tree_1[0],
                            file_lists_fault_tree_1[1]);
    }
    ...
    // Note: 1. Correspond to the Line 39-46 in test_fault_tree.py
    std::map<std::string, FTAUtils::FaultTree::_node *> tree_node_from_list = eventList();
    std::set<std::set<std::string>> tree_mocus_from_list;
    FTAUtils::FaultTree ft_from_list = FTAUtils::FaultTree(tree_mocus_from_list,
                                                           tree_node_from_list);
    ...
    FTAUtils::FaultTree ft_from_file_2 = FTAUtils::FaultTree(file_lists_fault_tree_2[0], 
                                                             file_lists_fault_tree_2[1]);
    FTAUtils::Parser parser_events_2 = FTAUtils::Parser(file_lists_fault_tree_2[0],
                                                        FTAUtils::Parser::FORMAT_CSV);

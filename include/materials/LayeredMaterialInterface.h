...
template <class T = Material> class LayeredMaterialInterface;
 ...
 */
template <class T> class LayeredMaterialInterface : public T {
public:
...
protected:
   ...
   * layer id.
   * @param param_name Name of the parameter to lookup according to layer id, it
   * must be the same length as "layer_ids"
  ...
  // connection between the "layer_id" and other layer parameters. These also
  // must be defined prior to the _layer_id,
  // which is intended for use in parent classes, because the getLayerParam
  // function that sets the reference for the
  // _layer_id uses these members.
private:
  /// The "input_ids" as entered in the input file, this is used for size
  /// checking of other layer parameters and
  ...
  /// A vector is used here that is sized to the maximum available layer_id
  /// value, this is inefficient with respect to
  /// to storage because the vector might be quite large depending with only a
  /// few non-zero entries. However, this
  /// vector must be accessed at each quadrature point, so doing some sort of
  /// look-up will impact performance, so we
  /// are sacrificing memory for performance.
  std::vector<unsigned int> _layer_id_to_param_index;

  /// The vector of data to reference and parameter data from the input
  /// parameters object.
  /// This uses a shared pointer rather than a unique pointer so it can be cast
  /// to the LayerParameter<P> type for
...
template <class T>
LayeredMaterialInterface<T>::LayeredMaterialInterface(
    const InputParameters &parameters)
    : T(parameters), _layer_variable(T::coupledValue("layer_variable")),
      _input_layer_ids(
          T::template getParam<std::vector<unsigned int>>("layer_ids")),
      _layer_id(getLayerParam<unsigned int>("layer_ids")) {
  if (!dynamic_cast<Material *>(this))
    mooseError("The LayeredMaterialInterface requires that the template class "
               "be a Material object.");

  const std::string &doc = parameters.getDocString("layer_ids");
  if (doc.find("[This should be modified in the parent classes") !=
      std::string::npos)
    mooseError("The documentation for the 'layer_ids' parameter must be "
               "modified using InputParameters::setDocString in the "
               "validParams function to include information on the parameters "
               "which the layer ids correspond.");
}

template <class T> void LayeredMaterialInterface<T>::computeProperties() {
  ...
  for (auto &data : _layer_data) {
    ...
    for (unsigned int qp = 0; qp < n; ++qp) {
      ...
      unsigned int current_layer_id =
          static_cast<unsigned int>(std::round(_layer_variable[qp]));
      ...
      if (idx == Mastodon::INVALID_LAYER_ID)
        mooseError("The current layer id variable value (", current_layer_id,
                   ") was not provided in the 'layer_ids' parameter of the \"",
                   T::name(), "\" block.");
...
LayeredMaterialInterface<T>::getLayerParam(const std::string &param_name) {
  ...
  if (data.size() != _input_layer_ids.size())
    mooseError(
        "The parameter \"", param_name, "\" in the \"", T::name(),
        "\" block must be the same length as the \"layer_ids\" parameter.");
...
LayeredMaterialInterface<T>::addLayerVector(const std::vector<P> &data) {
  // Initialize the layer id to index map, this is done here rather than the
  // constructor so that
  // the "layer_id" can utilize the getLayerParam function.
  if (_layer_id_to_param_index.empty()) {
    unsigned int n =
        *std::max_element(_input_layer_ids.begin(), _input_layer_ids.end());
  ...
  if (data.size() != _input_layer_ids.size())
    mooseError("The data in the \"", T::name(),
               "\" block supplied with the addLayerVector method must be the "
               "same length as the \"layer_ids\" parameter.");

  // Add the data to the list and return a reference to the MooseArray that will
  // be populated in computeProperties

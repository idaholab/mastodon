 ...
 *
 * It also defines a container that cannot be copied, which is the case with MooseArray objects
...
public:
  ...
  LayerParameter(const std::vector<P> & data) :
    _param_data(data)
  {
  }
   ...
   */
  virtual void resize(const unsigned int & n) override
  {
    _array_data.resize(n);
  }

  /**
   * Updates the MooseArray data at a quadrature point given the input parameter vector index.
